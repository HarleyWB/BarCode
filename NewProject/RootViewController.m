...
- (void)viewDidLoad
{
 ...
 //   overlayButton.backgroundColor=[UIColor greenColor];
   ...
   // [overlayButton setImage:[UIImage imageNamed:@"right.png"] forState:UIControlStateNormal];
    [overlayButton setFrame:CGRectMake(LineX( 85), LineY(430), LineX(290), LineY(60))];
    [overlayButton addTarget:self action:@selector(backButtonPressed1) forControlEvents:UIControlEventTouchUpInside];
...
//cvCopy(img, imgQR);
 ...
 //  cvFlip(imgQR,imgQR,1);
    ...
    ret = FftOpencvFun(img512, outBuf);/////调用检测函数 Fftopencvfun
  ...
  //  NSLog(@"%@", [all substringWithRange: rg]);
    ...
    if (ret)
    {
        if ([all length] > 10)
        {
            ...
            NSString* sResult = [all substringWithRange: rg];
            ...
            rg2.location = 5;
            rg2.length   = 5;
            NSString* sResult1 = [sResult substringWithRange: rg1];
 
            NSString* sResult2 = [sResult substringWithRange: rg2];
           
            
            if ([sResult1 isEqualToString: sResult2])///////判断前五位后五位是否相同
            {//labelshowstr 水印信息对应的结果。用于在主界面显示
                 [self playSound];
                /*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
                
                 NSString *LabelShowStr=[self getdatafromlist:sResult1];//使用水印结果 在字典中查询对应结果
                
               NSArray* array = [LabelShowStr componentsSeparatedByString:@"^"];
                NSRange rangeUrl = [ LabelShowStr rangeOfString:@"http"];
                ...
                if (LabelShowStr==nil){
                    ...
                    dispatch_async(main_que, ^{
                        ...
                        dispatch_async(main_que, ^{
                            ...
                            //      self.url=array[1];
                            // Update the
                            UILabel* labResult1= [[UILabel alloc] initWithFrame:CGRectMake(LineX( 85), LineY(430), LineX(290), LineY(30))];
                            labResult1.backgroundColor = [UIColor blackColor];
                            labResult1.numberOfLines=2;
                            labResult1.textColor=[UIColor whiteColor];
                            labResult1.text=sResult1;
                ...
                if (rangeUrl.location ==NSNotFound&&LabelShowStr!=nil){
                    dispatch_queue_t main_que = dispatch_get_main_queue();
                    dispatch_async(main_que, ^{
                        // Do some work
                        dispatch_async(main_que, ^{
                ...
                dispatch_async(main_que, ^{
                    ...
                    dispatch_async(main_que, ^{
                        ...
                        labResult1.textColor=[UIColor whiteColor];
                       labResult1.text=array[0];;
                       [labResult1 sizeToFit];
         
                            labResult1.numberOfLines=0;
                       [self.view addSubview:labResult1];
                        UILabel* labResult2= [[UILabel alloc] initWithFrame:CGRectMake(LineX( 85), LineY(460), LineX(290), LineY(30))];
                        labResult2.backgroundColor = [UIColor blackColor];
                        labResult2.numberOfLines=2;
                        labResult2.textColor=[UIColor whiteColor];
                        labResult2.text=array[1];;
                        [labResult2 sizeToFit];
                        [self.view addSubview:labResult2];
                         //[NSThread sleepForTimeInterval:2.0];
                         overlayButton.hidden=false;
                        //创建方式1
                  NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:4.0 target:self selector:@selector(timerac) userInfo:nil repeats:NO];
                        [UIView animateWithDuration:3 animations:^{labResult1.alpha = 0.0;} completion:^(BOOL finished)  {
                         [labResult1 removeFromSuperview];
                            [labResult2 removeFromSuperview];
                        }];
